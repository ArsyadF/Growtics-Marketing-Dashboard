    ...
    <div class="glass-card bg-white/95 dark:bg-slate-900/95 w-full max-w-sm p-6 md:p-8 rounded-3xl shadow-2xl border border-white/60 dark:border-blue-900/40 space-y-5 text-center relative z-[10000]">
      ...
      <form @submit.prevent="verify" class="space-y-4">
        ...
        <button 
          type="submit" 
          :disabled="loading" 
          class="w-full bg-theme-gradient hover:from-[#149b73] hover:to-[#149b73] text-white font-bold py-3 rounded-xl text-xs transition-all shadow-md flex items-center justify-center"
        >
      ...
      <div class="pt-2 border-t border-slate-200/50 dark:border-slate-800/80">
        <button type="button" @click="store.openModal('login')" class="text-xs text-theme hover:underline font-semibold cursor-pointer">

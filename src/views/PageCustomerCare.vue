...
<template>
  <div class="space-y-4 md:space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white/70 dark:bg-slate-900/70 backdrop-blur-md p-4 md:p-5 rounded-3xl border border-white/20 dark:border-slate-800 shadow-sm">
      <div>
        ...
        <p class="text-xs text-slate-400 mt-0.5">Kelola tiket aduan, masukan pelanggan, dan eskalasi penanganan masalah.</p>
      </div>
      ...
      <button 
        ...
        @click="openAddModal" 
        class="bg-theme-gradient hover:opacity-90 text-white font-bold px-4 py-2.5 rounded-2xl text-xs shadow-md transition-all flex items-center justify-center gap-2 cursor-pointer shrink-0"
      >
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <div class="glass-card bg-white/80 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs flex items-center gap-3">
        ...
        <div>
          <p class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Total Aduan</p>
          <p class="text-lg font-bold text-slate-800 dark:text-slate-100 leading-tight">{{ tickets.length }}</p>
        </div>
      </div>
      ...
      <div class="glass-card bg-white/80 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs flex items-center gap-3">
        ...
        <div>
          <p class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Menunggu</p>
          <p class="text-lg font-bold text-amber-600 dark:text-amber-400 leading-tight">{{ countByStatus('Open') }}</p>
        </div>
      </div>
      ...
      <div class="glass-card bg-white/80 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs flex items-center gap-3">
        ...
        <div>
          <p class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Diproses</p>
          <p class="text-lg font-bold text-purple-600 dark:text-purple-400 leading-tight">{{ countByStatus('In Progress') }}</p>
        </div>
      </div>
      ...
      <div class="glass-card bg-white/80 dark:bg-slate-900/80 p-4 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs flex items-center gap-3">
        ...
        <div>
          <p class="text-[10px] font-medium text-slate-400 uppercase tracking-wider">Selesai</p>
          <p class="text-lg font-bold text-emerald-600 dark:text-emerald-400 leading-tight">{{ countByStatus('Resolved') }}</p>
        </div>
      </div>
    </div>

    <!-- FILTER & PENCARIAN -->
    <div class="glass-card bg-white/80 dark:bg-slate-900/80 p-3.5 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-xs flex flex-col md:flex-row gap-3 justify-between items-center text-xs">
      ...
      <div class="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
        <select v-model="filterUnit" class="glass-input rounded-xl px-3 py-2 outline-none dark:bg-slate-800 font-medium">
          <option value="">Semua Unit</option>
          <option v-for="u in masterUnits" :key="u.code" :value="u.code">{{ u.code }}</option>
        </select>

        <select v-model="filterStatus" class="glass-input rounded-xl px-3 py-2 outline-none dark:bg-slate-800 font-medium">
        ...
        <select v-model="filterPriority" class="glass-input rounded-xl px-3 py-2 outline-none dark:bg-slate-800 font-medium">
    ...
    <transition enter-active-class="transition duration-200 ease-out" enter-from-class="opacity-0 -translate-y-2" enter-to-class="opacity-100 translate-y-0" leave-active-class="transition duration-150 ease-in" leave-from-class="opacity-100 translate-y-0" leave-to-class="opacity-0 -translate-y-2">
      <div v-if="selectedIds.length > 0" class="bg-emerald-600 text-white p-3 rounded-2xl flex flex-col md:flex-row items-center justify-between gap-3 shadow-lg text-xs">
        ...
        <div class="flex items-center gap-2 flex-wrap">
          ...
          <div v-if="canCreateOrEdit" class="flex items-center gap-1">
            ...
            <select v-model="bulkStatus" class="bg-emerald-700 text-white rounded-lg px-2.5 py-1 text-xs outline-none cursor-pointer border border-emerald-500 font-semibold">
          ...
          <div v-if="canCreateOrEdit" class="flex items-center gap-1">
            ...
            <select v-model="bulkPriority" class="bg-emerald-700 text-white rounded-lg px-2.5 py-1 text-xs outline-none cursor-pointer border border-emerald-500 font-semibold">
              <option value="">-- Tidak Diubah --</option>
              <option value="Tinggi">Tinggi</option>
              <option value="Sedang">Sedang</option>
              <option value="Rendah">Rendah</option>
          ...
          <button @click="resetBulkSelection" class="text-white/80 hover:text-white underline text-[11px] ml-1 cursor-pointer">
            Batal
          </button>
        </div>
      </div>
    ...
    <div class="glass-card bg-white/90 dark:bg-slate-900/90 rounded-3xl border border-slate-100 dark:border-slate-800/80 shadow-md overflow-hidden">
      <div class="overflow-x-auto">
        <table class="w-full text-left border-collapse text-xs">
          <thead>
            <tr class="bg-slate-100/70 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 border-b border-slate-200/60 dark:border-slate-800 select-none">
              <!-- Checkbox Select All -->
              ...
              <!-- Sort: No Tiket -->
              <th @click="sortBy('ticketNo')" class="p-3.5 font-bold cursor-pointer hover:text-emerald-600 transition-colors">
              ...
              <!-- Sort: Pelanggan -->
              <th @click="sortBy('customerName')" class="p-3.5 font-bold cursor-pointer hover:text-emerald-600 transition-colors">
                <div class="flex items-center gap-1">
              ...
              <!-- Sort: Unit -->
              <th @click="sortBy('unit')" class="p-3.5 font-bold cursor-pointer hover:text-emerald-600 transition-colors">
                <div class="flex items-center gap-1">
              ...
              <!-- Sort: Prioritas -->
              <th @click="sortBy('priority')" class="p-3.5 font-bold text-center cursor-pointer hover:text-emerald-600 transition-colors">
              ...
              <!-- Sort: Status -->
              <th @click="sortBy('status')" class="p-3.5 font-bold text-center cursor-pointer hover:text-emerald-600 transition-colors">
                <div class="flex items-center justify-center gap-1">
                  <span>Status</span>
              ...
              <!-- Sort: Tanggal -->
              <th @click="sortBy('date')" class="p-3.5 font-bold cursor-pointer hover:text-emerald-600 transition-colors">
                <div class="flex items-center gap-1">
                  <span>Tanggal</span>
          ...
          <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
            ...
            >
              ...
              <!-- Pelanggan -->
              <td class="p-3.5">
                <p class="font-bold text-slate-800 dark:text-slate-100 group-hover:text-theme transition-colors">{{ item.customerName }}</p>
                <p class="text-[10px] text-slate-400">{{ item.contact || '-' }}</p>
              </td>

              <!-- Unit -->
              <td class="p-3.5 font-semibold">
                <span class="px-2 py-1 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200/50 dark:border-slate-700/50">
                  {{ item.unit }}
                </span>
              </td>
              ...
              <td class="p-3.5 max-w-xs">
                <p class="font-semibold text-slate-700 dark:text-slate-200 truncate">{{ item.category }}</p>
                <p class="text-[11px] text-slate-400 truncate">{{ item.description }}</p>
              </td>

              <!-- Prioritas Badge -->
              <td class="p-3.5 text-center">
                <span :class="getPriorityClass(item.priority)" class="px-2.5 py-1 rounded-full text-[10px] font-bold">
                  {{ item.priority }}
                </span>
              </td>
              ...
              <!-- Status Badge -->
              <td class="p-3.5 text-center">
                <span :class="getStatusClass(item.status)" class="px-2.5 py-1 rounded-full text-[10px] font-bold inline-flex items-center gap-1">
              ...
              <td class="p-3.5 text-center" @click.stop>
                <div class="flex items-center justify-center gap-1.5">
                  <button 
                    ...
                    :title="canCreateOrEdit ? 'Edit Tiket' : 'Lihat Detail Tiket'"
                  >
                    <i :class="canCreateOrEdit ? 'fa-solid fa-pen-to-square' : 'fa-solid fa-eye'"></i>
                  </button>
            ...
            <tr v-if="filteredTickets.length === 0">
              <td colspan="9" class="p-8 text-center text-slate-400">
                ...
                <p class="font-medium">Tidak ada data tiket aduan yang ditemukan.</p>
              </td>
            </tr>
          </tbody>
        </table>
    ...
    <Teleport to="body">
      <div v-if="isModalOpen" class="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4" @click.self="isModalOpen = false">
        <div class="glass-card w-full max-w-lg bg-white/95 dark:bg-slate-900/95 rounded-3xl p-6 shadow-2xl space-y-4 border border-slate-100 dark:border-slate-800 max-h-[90vh] overflow-y-auto">
          
          <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
            <div>
              <span class="text-[10px] font-mono font-bold text-theme bg-theme/10 px-2 py-0.5 rounded border border-theme/20">
                #{{ form.ticketNo }}
              </span>
              <h3 class="font-bold text-slate-800 dark:text-slate-100 text-sm md:text-base mt-1 flex items-center gap-2">
                <i class="fa-solid fa-ticket text-theme"></i>
                {{ isEdit ? (canCreateOrEdit ? 'Edit Tiket Aduan' : 'Rincian Tiket Aduan') : 'Buat Tiket Aduan Baru' }}
              </h3>
            </div>
            <button @click="isModalOpen = false" class="text-slate-400 hover:text-slate-600 cursor-pointer">
          ...
          <form @submit.prevent="saveTicket" class="space-y-3 text-xs">
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-slate-500 font-medium mb-1">Nama Pelanggan / Pelapor</label>
                <input 
              ...
              <div>
                <label class="block text-slate-500 font-medium mb-1">Kontak (WA / Email)</label>
                <input 
            ...
            <div class="grid grid-cols-2 gap-3">
              ...
              <div>
                <label class="block text-slate-500 font-medium mb-1">Kategori Masalah</label>
                <select 
                ...
                >
                  ...
                  <option value="Kerusakan Produk / Cetak">Kerusakan Produk / Cetak</option>
                  <option value="Pelayanan CS / Admin">Pelayanan CS / Admin</option>
                  <option value="Kesalahan Order / Jumlah">Kesalahan Order / Jumlah</option>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-slate-500 font-medium mb-1">Tingkat Prioritas</label>
                <select 
              ...
              <div>
                <label class="block text-slate-500 font-medium mb-1">Status Penanganan</label>
                <select 
                  v-model="form.status" 
                  required 
                  :disabled="!canCreateOrEdit" 
            ...
            <div>
              <label class="block text-slate-500 font-medium mb-1">Rincian Keluhan / Aduan</label>
              <textarea 
            ...
            <!-- Info Pembuat Tiket -->
            <div v-if="form.createdBy" class="text-[10px] text-slate-400 italic">
            ...
            <div class="pt-3 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
              ...
              <div class="flex gap-2 ml-auto">
                <button type="button" @click="isModalOpen = false" class="px-4 py-2 rounded-xl text-slate-500 font-bold hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer">
                ...
                <button 
                  ...
                  class="bg-theme-gradient text-white px-5 py-2 rounded-xl font-bold shadow-md cursor-pointer disabled:opacity-50"
                >
...
const applyBulkEdit = async () => {
  ...
  if (!bulkStatus.value && !bulkPriority.value) {
    store.addNotification('Peringatan', 'Pilih minimal satu perubahan (Status atau Prioritas)', 'warning');
    return;
  }
  ...
  try {
    ...
    if (bulkPriority.value) changesText.push(`Prioritas: ${bulkPriority.value}`);
...
const canDeleteItem = (item) => {
  ...
  if (store.canEditPage('aduan')) {
    ...
    return (userEmail && creator === userEmail) || (userId && creator === userId);
  }
...
const countByStatus = (st) => tickets.value.filter(t => t.status === st).length;
...
const getSortIcon = (field) => {
  ...
  return sortOrder.value === 'asc' ? 'fa-sort-up text-emerald-600' : 'fa-sort-down text-emerald-600';
};
...
const bulkUpdateStatus = async (newStatus) => {
  ...
  try {
    ...
    await Promise.all(updatePromises);
    store.addNotification('Berhasil', `Status ${selectedIds.value.length} tiket diperbarui menjadi ${newStatus}`, 'success');
    selectedIds.value = [];
  } catch (err) {
    store.addNotification('Gagal', err.message, 'warning');
  } finally {
...
const bulkUpdatePriority = async (newPriority) => {
  ...
  try {
    ...
    await Promise.all(updatePromises);
    store.addNotification('Berhasil', `Prioritas ${selectedIds.value.length} tiket diperbarui menjadi ${newPriority}`, 'success');
    selectedIds.value = [];
  } catch (err) {
    store.addNotification('Gagal', err.message, 'warning');
  } finally {
...
const bulkDelete = () => {
  ...
  const targetTickets = tickets.value.filter(t => selectedIds.value.includes(t.id));
  ...
  if (deletableTickets.length === 0) {
    store.addNotification('Akses Ditolak', `Seluruh ${targetTickets.length} tiket yang dicentang bukan milik Anda dan tidak dapat dihapus.`, 'warning');
    return;
  }
  ...
  store.openAlert('Konfirmasi Hapus Massal', confirmMsg, async () => {
    store.isLoading = true;
    try {
      ...
      deletableTickets.forEach(t => {
        const idx = store.db.aduanList.findIndex(item => String(item.id) === String(t.id));
      ...
      if (forbiddenCount > 0) {
        ...
        store.addNotification('Berhasil', `${deletableTickets.length} tiket aduan berhasil dihapus.`, 'success');
      }
...
const getPriorityClass = (priority) => {
  switch (priority) {
    case 'Tinggi': return 'bg-rose-500/10 text-rose-600 border border-rose-500/20';
    case 'Sedang': return 'bg-amber-500/10 text-amber-600 border border-amber-500/20';
...
const getStatusClass = (status) => {
  switch (status) {
    ...
    case 'In Progress': return 'bg-purple-500/10 text-purple-600 border border-purple-500/20';
...
const saveTicket = async () => {
  ...
  try {
    const payload = { 
      ...form,
      createdBy: form.createdBy || currentUser.value?.email || currentUser.value?.nama || ''
    };
    ...
    if (isEdit.value) {
      const idx = store.db.aduanList.findIndex(t => String(t.id) === String(form.id));
      if (idx !== -1) store.db.aduanList[idx] = payload;
    } else {
      store.db.aduanList.unshift(payload);
    }

    store.addNotification('Disimpan', `Tiket #${form.ticketNo} berhasil disimpan`, 'success');
...
const deleteTicket = (item) => {
  if (!canDeleteItem(item)) {
    store.addNotification('Akses Ditolak', 'Anda hanya dapat menghapus tiket aduan yang Anda buat sendiri.', 'warning');
    return;
  }

  store.openAlert('Konfirmasi Hapus', `Hapus tiket aduan #${item.ticketNo}?`, async () => {
    try {
      if (api && api.deleteAduanData) await api.deleteAduanData(item.id);
      const idx = store.db.aduanList.findIndex(t => String(t.id) === String(item.id));
      if (idx !== -1) store.db.aduanList.splice(idx, 1);
      
      store.addNotification('Dihapus', 'Tiket aduan telah dihapus', 'warning');
      isModalOpen.value = false;
    } catch (err) {
      store.addNotification('Gagal', err.message, 'warning');
    }

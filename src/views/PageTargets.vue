...
<template>
  <section id="page-targets" class="page-section space-y-4 md:space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h2
          class="text-base md:text-lg font-bold text-slate-800 dark:text-slate-100"
        >
          Rekapitulasi Target vs Realisasi
        </h2>
        <p class="text-slate-400 text-xs">
          Pantau progres ketercapaian target revenue per unit bisnis.
        </p>
      </div>
      <button
          ...
          <tbody class="divide-y dark:divide-slate-800/80">
            <tr v-if="store.isLoading">
              <td colspan="5" class="text-center py-6 text-slate-400">
                Memuat data target & realisasi...
              </td>
            </tr>
            <tr
              v-else
              v-for="t in listTargets"
              :key="t.name"
              class="hover:bg-white/20 dark:hover:bg-slate-800/40"
            >
              <td
                class="py-3.5 px-4 font-bold text-slate-800 dark:text-slate-100"
              >
                {{ t.name }}
              </td>
              ...
              <td class="py-3.5 px-4 text-theme font-bold">
                {{ formatRp(t.real) }}
              </td>
              ...
              <td class="py-3.5 px-4">
                <div class="flex items-center gap-2">
                  ...
                  <!-- Progress Bar Mini -->
                  <div
                    class="w-20 bg-slate-200 dark:bg-slate-700 h-1.5 rounded-full overflow-hidden hidden sm:block"
                  >
                    <div

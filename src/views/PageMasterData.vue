...
<template>
  <div class="space-y-4 md:space-y-6">
    ...
    <div class="glass-card p-4 md:p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        ...
        <p class="text-[11px] md:text-xs text-slate-400 mt-0.5">
          Kelola entitas utama secara independen. Perubahan pada tiap bagian disave secara terpisah.
        </p>
      </div>
    </div>

    <!-- SECTION 1: MASTER TARGET REVENUE (GLOBAL & UNIT) -->
    <div class="glass-card p-4 md:p-5 rounded-2xl space-y-4 border border-emerald-500/20 bg-emerald-500/5">
      <div class="flex items-center justify-between border-b border-emerald-500/20 pb-3">
        <h4 class="font-bold text-xs md:text-sm text-slate-800 dark:text-slate-100 flex items-center gap-2">
        ...
        <button 
          ...
          type="button"
          class="bg-theme-gradient hover:bg-theme-gradient text-white font-bold px-3.5 py-1.5 rounded-xl text-xs shadow-sm transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
        >
          <i class="fa-solid fa-floppy-disk" :class="{ 'fa-spin': savingState.targets }"></i>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
        <div>
          <label class="block text-slate-500 font-semibold mb-1">Target Global (1 Tahun):</label>
          <input 
        ...
        <div>
          <label class="block text-slate-500 font-semibold mb-1">Target Unit NHP:</label>
          <input 
        ...
        <div>
          <label class="block text-slate-500 font-semibold mb-1">Target Unit NHC:</label>
          <input 
        ...
        <div>
          <label class="block text-slate-500 font-semibold mb-1">Target Unit KG:</label>
          <input 
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6 items-start">
      ...
      <!-- 2. MASTER UNIT USAHA -->
      <div class="glass-card p-4 rounded-2xl space-y-4 border border-white/50 dark:border-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
          ...
          >
            <i class="fa-solid fa-floppy-disk" :class="{ 'fa-spin': savingState.units }"></i>
        ...
        <form @submit.prevent="addUnit" class="flex gap-2">
          ...
          >
          <button type="submit" class="bg-blue-600 hover:bg-blue-700 text-white font-bold px-3 py-1.5 rounded-xl text-xs shadow-sm cursor-pointer">
        ...
        <div class="space-y-2 max-h-[360px] overflow-y-auto pr-1">
          ...
          >
            <div>
              <span class="font-black text-blue-600 dark:text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded text-[10px] mr-1.5 uppercase border border-blue-500/20">
                {{ u.code }}
              </span>
              <span class="font-semibold text-slate-700 dark:text-slate-200">{{ u.name }}</span>
            </div>
            <button @click="removeUnit(idx)" type="button" class="text-slate-400 hover:text-rose-500 p-1 cursor-pointer">
      ...
      <div class="glass-card p-4 rounded-2xl space-y-4 border border-white/50 dark:border-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
          ...
          >
            <i class="fa-solid fa-floppy-disk" :class="{ 'fa-spin': savingState.divisions }"></i>
        ...
        <form @submit.prevent="addDivisi" class="flex gap-2">
          <input 
            ...
            class="flex-1 glass-input rounded-xl px-2.5 py-1.5 text-xs outline-none dark:bg-slate-800 text-slate-800 dark:text-slate-100"
          >
          <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3 py-1.5 rounded-xl text-xs shadow-sm cursor-pointer">
            <i class="fa-solid fa-plus"></i>
          </button>
        </form>
        ...
        <div class="space-y-2 max-h-[360px] overflow-y-auto pr-1">
          <div 
            ...
            class="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-xs"
          >
            <span class="font-semibold text-slate-700 dark:text-slate-200">{{ d }}</span>
            <button @click="removeDivisi(idx)" type="button" class="text-slate-400 hover:text-rose-500 p-1 cursor-pointer">
              <i class="fa-solid fa-trash-can text-xs"></i>
            </button>
          </div>
        </div>
      ...
      <div class="glass-card p-4 rounded-2xl space-y-4 border border-white/50 dark:border-slate-800">
        <div class="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-2.5">
          ...
          >
            <i class="fa-solid fa-floppy-disk" :class="{ 'fa-spin': savingState.platforms }"></i>
        ...
        <form @submit.prevent="addPlatform" class="flex gap-2">
          <input 
            ...
            class="flex-1 glass-input rounded-xl px-2.5 py-1.5 text-xs outline-none dark:bg-slate-800 text-slate-800 dark:text-slate-100"
          >
          <button type="submit" class="bg-purple-600 hover:bg-purple-700 text-white font-bold px-3 py-1.5 rounded-xl text-xs shadow-sm cursor-pointer">
            <i class="fa-solid fa-plus"></i>
          </button>
        </form>
        ...
        <div class="space-y-2 max-h-[360px] overflow-y-auto pr-1">
          <div 
            ...
            class="p-2.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200/60 dark:border-slate-700/60 flex items-center justify-between text-xs"
          >
            <span class="font-semibold text-slate-700 dark:text-slate-200">{{ p }}</span>
            <button @click="removePlatform(idx)" type="button" class="text-slate-400 hover:text-rose-500 p-1 cursor-pointer">
              <i class="fa-solid fa-trash-can text-xs"></i>
            </button>
          </div>
        </div>
...
const addUnit = () => {
  ...
  unitList.value.push({ code: newUnitCode.value.trim().toUpperCase(), name: newUnitName.value.trim() });
  newUnitCode.value = '';
  newUnitName.value = '';
};
const removeUnit = (idx) => { unitList.value.splice(idx, 1); };
...
const removeDivisi = (idx) => { divisiList.value.splice(idx, 1); };
...
const removePlatform = (idx) => { platformList.value.splice(idx, 1); };
...
const saveTargetsOnly = async () => {
  ...
  try {
    ...
    store.addNotification('Target Disimpan', 'Master target revenue berhasil diperbarui', 'success');
  } catch (err) {
...
const saveUnitsOnly = async () => {
  ...
  try {
    ...
    store.addNotification('Unit Disimpan', 'Master unit usaha berhasil diperbarui', 'success');
  } catch (err) {
    store.addNotification('Gagal Simpan', err.message, 'warning');
  } finally {
...
const saveDivisionsOnly = async () => {
  ...
  try {
    ...
    store.addNotification('Divisi Disimpan', 'Master divisi berhasil diperbarui', 'success');
  } catch (err) {
    store.addNotification('Gagal Simpan', err.message, 'warning');
  } finally {
...
const savePlatformsOnly = async () => {
  ...
  try {
    ...
    store.addNotification('Platform Disimpan', 'Master platform sales berhasil diperbarui', 'success');
  } catch (err) {
    store.addNotification('Gagal Simpan', err.message, 'warning');
  } finally {

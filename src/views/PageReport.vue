...
<template>
  <div class="space-y-4 md:space-y-6">
    ...
    >
      <div>
        ...
        <p class="text-[11px] md:text-xs text-slate-400 mt-0.5">
          Generator template laporan terstruktur untuk kebutuhan rapat evaluasi
          dan presentasi direksi.
        </p>
      </div>
      ...
      <div class="flex items-center gap-2">
        <span
          class="text-[10px] font-bold uppercase px-3 py-1 rounded-full border text-theme bg-button/10 border-theme/20"
        >
    ...
    <div class="glass-card p-4 rounded-2xl space-y-3">
      <h4
        class="font-bold text-slate-800 dark:text-slate-100 text-xs uppercase tracking-wider flex items-center gap-2"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <div
        class="glass-card p-4 md:p-5 rounded-2xl border border-white/20 dark:border-slate-800 hover:border-theme/40 transition-all space-y-3 flex flex-col justify-between"
      >
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <span
              class="p-2 bg-emerald-500/10 text-emerald-500 rounded-xl text-base"
            >
              <i class="fa-solid fa-chart-line"></i>
            </span>
            <span class="text-[9px] font-bold text-slate-400 uppercase"
              >Template PDF / Excel</span
            >
          </div>
          <h4
            class="font-bold text-slate-800 dark:text-slate-100 text-sm md:text-base"
          >
            Laporan Kinerja Finansial & Omzet
          </h4>
          <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Rekapitulasi total revenue terfilter vs target tahunan, selisih
            tahun lalu, dan breakdown omzet per platform & divisi.
          </p>
        </div>

        <div
          class="pt-3 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between gap-2"
        >
          <button
            @click="generateReport('finansial')"
            class="flex-1 bg-button text-white py-2 rounded-xl text-xs font-bold shadow-md hover:opacity-95 transition-all cursor-pointer flex items-center justify-center gap-1.5"
          >
      ...
      >
        <div class="space-y-2">
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Analisis efisiensi biaya iklan vs perolehan leads, rasio follow up,
            dan persentase closing transaksi per unit usaha.
          </p>
        </div>
      ...
      >
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <span
              class="p-2 bg-amber-500/10 text-amber-500 rounded-xl text-base"
            >
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Status pencapaian program kerja triwulanan divisi produksi, daftar
            program terlewat, dan evaluasi PIC penanggung jawab.
          </p>
        </div>
      ...
      >
        <div class="space-y-2">
          ...
          <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
            Ringkasan mingguan jangkauan (reach), engagement rate (ER%), dan
            konten paling berprestasi (content winners).
          </p>
        </div>
    ...
    <Teleport to="body">
      ...
      >
        <div
          class="w-full max-w-2xl glass-card bg-white dark:bg-slate-900 rounded-3xl p-6 shadow-2xl space-y-4 border border-slate-100 dark:border-slate-800 max-h-[90vh] overflow-y-auto"
        >
          <!-- Header Preview -->
          <div
          ...
          >
            <div>
              ...
              <p class="text-[10px] text-slate-400 mt-0.5">
                Dokumen siap dicetak atau diunduh
              </p>
            </div>
            <button
          ...
          <div
            class="p-5 bg-slate-50 dark:bg-slate-800/60 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-4 font-mono text-xs text-slate-700 dark:text-slate-200"
          >
            <div
              class="text-center border-b pb-3 border-slate-300 dark:border-slate-700"
            >
              <h2
                class="font-bold text-sm text-slate-900 dark:text-white uppercase tracking-wider"
              >
                LAPORAN EKSKUTIF PERFORMA {{ previewData.type.toUpperCase() }}
              </h2>
              <p class="text-[10px] text-slate-500 dark:text-slate-400 mt-0.5">
                Cakupan: Unit {{ reportFilter.unit }} | Periode:
                {{ reportFilter.startDate }} s/d {{ reportFilter.endDate }}
              </p>
            </div>
            ...
            <div class="space-y-2">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-emerald-500 font-bold"
                  >TERVERIFIKASI SISTEM</span
                >
              </div>
            </div>

            <div
              class="p-3 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 text-[11px] leading-relaxed"
            >
              <strong>Ringkasan Eksekutif:</strong><br />
              Laporan ini memuat ringkasan performa operasional dan finansial
              terhitung sejak tanggal {{ reportFilter.startDate }} hingga
              {{ reportFilter.endDate }}. Seluruh angka disintesis secara
              otomatis oleh sistem Marketing v3.0.
            </div>
          </div>
          ...
          <div class="flex justify-end gap-2 pt-2">
            ...
            <button
              @click="printDocument"
              class="bg-button text-white px-5 py-2 rounded-xl text-xs font-bold shadow-md hover:opacity-95 transition-all cursor-pointer flex items-center gap-1.5"
            >
...
const userAccessRoleText = computed(() => {
  ...
  const unit = store.currentUser?.aksesUnit
    ? store.currentUser.aksesUnit.join(", ")
    : store.currentUser?.unit || "NHP";
...
onMounted(() => {
  ...
  reportFilter.startDate =
    store.filterDates?.start || `${new Date().getFullYear()}-01-01`;
  reportFilter.endDate =
    store.filterDates?.end || new Date().toISOString().split("T")[0];
...
const printDocument = () => {
  ...
  store.addNotification(
    "Laporan Diekspor",
    `Dokumen Laporan ${previewData.type} berhasil dicetak`,
    "success",
  );
};
</script>
